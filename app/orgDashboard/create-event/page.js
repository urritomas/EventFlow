...
const [location, setLocation] = useState({ lat: null, lng: null });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        if (!formData.eventName.trim()) throw new Error("Event name is required");
        ...
        const eventPayload = {
            ...
            geofence_checkin_window:
                (parseInt(formData.geofenceEarlyMinutes || "0", 10) || 0) +
                (parseInt(formData.geofenceLateMinutes || "0", 10) || 0),
            geofence_checkout_window:
                (parseInt(formData.geofenceEarlyCheckoutMinutes || "0", 10) || 0) +
                (parseInt(formData.geofenceLateCheckoutMinutes || "0", 10) || 0),
            ...
            org_login_id: orgLoginId ? parseInt(orgLoginId, 10) : null,
        };
        ...
        setFormData({
            ...
            venue: "", fullAddress: "", expectedAttendance: "", description: "",
            rfidEnabled: false, faceRecognitionEnabled: false, geofencingEnabled: false,
            geofenceEarlyMinutes: "", geofenceLateMinutes: "", geofenceLateCheckinThreshold: "15",
            geofenceReverifyMinutes: "15", geofenceEarlyCheckoutMinutes: "", geofenceLateCheckoutMinutes: "",
        });
        setLocation({ lat: null, lng: null });

        setTimeout(() => router.push("/orgDashboard"), 1500);
